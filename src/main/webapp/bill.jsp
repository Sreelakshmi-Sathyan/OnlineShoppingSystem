...
try
{
	...
	ResultSet rs=stmt.executeQuery("select sum(total) from cart where email='"+email+"' and status='bill' and transactionId ='" + transactionId +"'");
	while(rs.next())
	{
		total=rs.getInt(1);
	}
	ResultSet rs1=stmt.executeQuery("select * from users inner join cart where users.email='"+email+"' and cart.status='bill'");
	while(rs1.next())
	{
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs2=stmt.executeQuery("select *from cart inner join product where cart.product_id=product.id and cart.email='"+email+"'and cart.status='bill' and transactionId ='" + transactionId +"'");
  while(rs2.next())
  {
	  sno=sno+1;
